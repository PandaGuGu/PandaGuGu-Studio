import { createContext, useContext } from 'react'

export type Lang = 'zh-CN' | 'en'

const translations: Record<Lang, Record<string, string>> = {
  'zh-CN': {
    'header.subtitle': '画布试验场',
    'header.whatIsThis': '这是什么?',
    'header.openSettings': '设置',
    'header.model': '模型',
    'header.modelNotSet': '模型未配置',
    'header.tipProvider': '服务商',
    'header.tipVision': '能力',
    'header.tipVisionYes': '视觉理解 ✓',
    'header.tipEndpoint': '端点',
    'header.tipNotSet': '尚未配置模型服务。点击这里打开设置，选择服务商并粘贴 API Key。',
    'header.tipOpenSettings': '点击打开设置',
    'header.aboutTitle': '画布试验场',
    'header.aboutDesc': '一个面向视觉大模型的自由画布。画草图、贴参考、加标注,然后让模型看懂并构建。',
    'header.aboutProviders': '支持的模型',
    'header.aboutHow': '使用方式',
    'header.aboutStep1T': '画或导入',
    'header.aboutStep1D': '绘制线框草图、粘贴截图、拖入参考图片。',
    'header.aboutStep2T': '描述',
    'header.aboutStep2D': '补充一段文字说明你想要的页面。',
    'header.aboutStep3T': '生成',
    'header.aboutStep3D': '模型看到画布 + 描述,流式生成 HTML/CSS/JS。',
    'header.aboutStep4T': '细化',
    'header.aboutStep4D': '迭代优化。模型会结合草图、输出截图和你的反馈继续修改。',
    'header.aboutFeatures': '功能',
    'header.aboutF1': '整画布输出或按画框区域选择',
    'header.aboutF2': '实时流式生成,带 token 可视化',
    'header.aboutF3': '规划模式:观察 → 构思 → 创建',
    'header.aboutF4': '画作保存与载入',
    'header.aboutFooter1': '点击右上角模型按钮切换模型服务商',
    'header.aboutFooter2': '由 PandaGuGu Studio 构建',
    'theme.dark': '深色',
    'theme.light': '浅色',
    'theme.toggleDark': '切换为深色背景',
    'theme.toggleLight': '切换为浅色背景',

    'prompt.inspirationArt': '生成艺术',
    'prompt.useBlueprint': '用画布蓝图生成',
    'prompt.useBlueprintHint': '把当前画布的 JSON 蓝图填入提示词,让 AI 按蓝图生成 HTML',
    'prompt.inspirationApp': '线框 → 应用',
    'prompt.inspirationLanding': '落地页',
    'prompt.inspirationDash': '数据看板',
    'prompt.placeholderNew': '描述你想构建什么…',
    'prompt.placeholderRefine': '描述要修改的地方,或留空直接点“细化”…',
    'prompt.planOn': '规划模式已开启 — 观察、构思、创建',
    'prompt.planOff': '开启规划模式,进行更深度的多步生成',
    'prompt.planLabel': '规划',
    'prompt.planHint': '观察 · 构思 · 创建',
    'prompt.clear': '清空',
    'prompt.noKey': '未配置 Key',
    'prompt.refine': '细化',
    'prompt.generate': '生成',
    'prompt.hintNoKey': '请先在上方配置 API Key',

    'frame.fullCanvas': '将发送整个画布给 AI',
    'frame.drawFirst': '先在画布上画点什么',
    'frame.prevOutput': '上一次输出',
    'frame.add': '画框',
    'frame.save': '保存',
    'frame.load': '载入',
    'frame.sources': '素材区',
    'frame.all': '全选',
    'frame.lastOutput': '上次输出',
    'frame.preview': '预览',
    'frame.collapse': '折叠',
    'frame.expand': '展开',
    'frame.frames': '个画框',

    'layers.title': '图层',
    'layers.add': '添加画框',
    'layers.delete': '删除该图层',
    'layers.empty': '画布为空,画一个元素试试',
    'frame.exportJson': '导出该画框的 JSON',
    'frame.title': '画框',
    'frame.adjustHint': '画框尺寸改变时,框内元素不会自动缩放',
    'template.title': '经典模板库',
    'template.desc': '选择模板后自动在画布上生成画框 + 语义元素，可直接编辑',
    'template.mobile': '手机',
    'template.desktop': '电脑',
    'template.appHome': 'App 首页',
    'template.appLogin': 'App 登录页',
    'template.saasLanding': 'SaaS 落地页',
    'template.dashboard': '数据仪表盘',
    'template.ecommerce': '电商首页',
    'template.portfolio': '个人主页',

    'app.apiKeyTitle': '需要配置 API Key',
    'app.apiKeyDesc': '从 {url} 获取 {provider} 的 API Key。',
    'app.apiKeyStep1': '从 {url} 获取 Key',
    'app.apiKeyStep2': '点击右上角模型按钮打开设置',
    'app.apiKeyStep3': '在 {provider} 卡片中粘贴 Key',
    'app.openSettings': '打开设置',
    'app.copy': '复制',
    'app.openNew': '新窗口打开',
    'app.download': '下载 HTML',
    'variant.title': '批量变体',
    'variant.presets': '预置风格',
    'variant.customPlaceholder': '自定义风格描述，如: 日式杂志排版…',
    'variant.add': '添加',
    'variant.generate': '批量生成 {n} 个变体',
    'variant.generating': '正在生成…',
    'variant.failed': '所有变体生成失败，请检查网络或 API Key',
    'variant.barLabel': '变体',
    'variant.download': '下载该变体 HTML',
    'variant.exit': '退出变体预览',
    'variant.minimal': '极简',
    'variant.cyberpunk': '赛博朋克',
    'variant.morandi': '莫兰迪',
    'variant.glass': '玻璃拟态',
    'variant.terminal': '暗黑终端',
    'variant.neumorphic': '新拟物',
    'variant.luxury': '高端质感',
    'variant.retro': '复古像素',
    'history.title': '生成历史',
    'history.empty': '还没有历史记录，生成一次就会自动存档',
    'history.load': '载入此版本到预览',
    'history.download': '下载此版本 HTML',
    'history.delete': '删除此记录',
    'history.clearAll': '清空全部历史',
    'import.title': '导入 HTML 到画布',
    'import.desc': '粘贴 HTML 或选择文件。导入时会简化成语义元素（div→容器、h1→标题、button→按钮…）并自动布局到画布，可直接编辑。',
    'import.file': '选择 HTML 文件',
    'import.hint': '或直接粘贴下方',
    'import.placeholder': '<!DOCTYPE html>\n<html>\n  <body>\n    <div class="hero">\n      <h1>Hello</h1>\n      <button>Go</button>\n    </div>\n  </body>\n</html>',
    'import.confirm': '导入到画布（将清空当前画布）',
    'import.cancel': '取消',
    'import.empty': '请输入 HTML 内容',
    'import.parseError': '无法解析 HTML，请检查内容格式',
    'import.importing': '导入中…',
    'import.readError': '文件读取失败',
    'app.desktop': '桌面',
    'app.tablet': '平板',
    'app.mobile': '手机',
    'app.phaseGaze': '观察',
    'app.phaseDream': '构思',
    'app.phaseCreate': '创建',
    'app.chipFullCanvas': '整个画布',
    'app.chipCurrentOutput': '当前输出',

    'settings.title': '设置',
    'settings.general': '常规设置',
    'settings.language': '界面语言',
    'settings.openaiCompat': 'OpenAI 兼容',
    'settings.active': '使用中',
    'settings.url': '地址',
    'settings.searchModels': '搜索视觉模型…',
    'settings.loading': '加载中…',
    'settings.error': '出错',
    'settings.modelIdPlaceholder': '输入模型 ID…',
    'settings.cancel': '取消',
    'settings.save': '保存',

    'stream.thinking': '思考过程',
    'stream.complete': '完成',
    'stream.thinkingNow': '思考中',
    'stream.streaming': '生成中',
    'stream.chars': '字符',

    'plan.gazing': '正在观察图像…',
    'plan.dreaming': '正在构思设计…',
    'plan.creating': '正在编写代码…',
    'plan.working': '工作中…',
    'plan.phase': '阶段',

    'semantic.title': '语义标记',
    'semantic.autoTag': '智能打标',
    'semantic.drawHint': '未选中元素 — 点击后在画布上拖拽创建该类型',
    'align.title': '对齐',
    'align.needMulti': '请先框选或 Shift+点击选中多个元素',
    'align.left': '左对齐',
    'align.hcenter': '水平居中',
    'align.right': '右对齐',
    'align.top': '顶对齐',
    'align.vcenter': '垂直居中',
    'align.bottom': '底对齐',
    'align.hdistribute': '水平等距分布',
    'align.vdistribute': '垂直等距分布',
    'semantic.container': '容器',
    'semantic.section': '区段',
    'semantic.card': '卡片',
    'semantic.nav': '导航',
    'semantic.heading': '标题',
    'semantic.text': '文本',
    'semantic.link': '链接',
    'semantic.button': '按钮',
    'semantic.input': '输入框',
    'semantic.image': '图片',
    'semantic.raw': 'HTML 片段',
    'semantic.note': '便签',
    'semantic.groupContainer': '容器',
    'semantic.groupContent': '内容',
    'semantic.groupControl': '控件',
    'semantic.groupMedia': '媒体',
    'semantic.groupSpecial': '特殊',
    'semantic.untag': '清除标记',
    'semantic.search': '搜索并快速标记',
    'semantic.searchPlaceholder': '搜索类型…',
    'semantic.searchEmpty': '无匹配类型',
    'semantic.exportBlueprint': '导出 JSON（图片嵌数据）',
    'semantic.exportBlueprintFile': '导出 JSON（图片用文件名）',
    'semantic.exportBlueprintFileHint': '图片用文件名引用（如 image-1.png），不嵌入 base64，适合把图片文件单独传给 AI',
    'semantic.exportHtml': '导出 HTML（图片内嵌）',
    'semantic.exportHtmlHint': '直接生成自包含 HTML，图片 base64 内嵌，双击即可看，无需 AI 参与',
    'semantic.copyForAI': '复制给网页 AI',
    'semantic.copyForAIHint': '复制提示词+JSON（已说明 src 是 base64 字符串、不要解码、原样复制）',
    'semantic.exportPng': '导出画布 PNG',
    'semantic.exportMenu': '导出选项',
    'semantic.exportEmpty': '画布上还没有语义标记。先选中一个图形,再点类型按钮打标。',
    'semantic.importHtml': '导入 HTML',
    'semantic.templates': '模板库',
    'semantic.propsHint': '属性',
    'semantic.layout': '布局',
    'semantic.layoutFree': '自由定位',
    'semantic.layoutRow': '横排',
    'semantic.layoutColumn': '竖排',
    'semantic.layoutGrid': '网格',
    'semantic.layoutWrap': '自动换行',
    'semantic.advanced': '高级',
    'semantic.style': '自由 CSS (style)',
    'semantic.stylePlaceholder': 'display:flex; gap:16px; backdrop-filter:blur(10px)',
    'semantic.events': '事件 (events JSON)',
    'semantic.htmlHint': '此类型的 html 字段在主表单里直接编辑。',
    'props.label': '标签',
    'props.width': '宽度',
    'props.height': '高度',
    'props.bg': '背景',
    'props.radius': '圆角',
    'props.padding': '内边距',
    'props.border': '边框',
    'props.shadow': '阴影',
    'props.content': '内容',
    'props.fontSize': '字号',
    'props.fontWeight': '字重',
    'props.color': '颜色',
    'props.bold': '加粗',
    'props.align': '对齐',
    'props.alignLeft': '左',
    'props.alignCenter': '中',
    'props.alignRight': '右',
    'props.alt': '描述',
    'props.fileName': '文件名',
    'props.src': '图片来源',
    'props.href': '链接地址',
    'props.placeholder': '占位符',
    'props.type': '类型',
    'props.level': '级别',
    'props.fit': '填充',
    'props.html': 'HTML 片段',
  },

  en: {
    'header.subtitle': 'PLAYGROUND',
    'header.whatIsThis': 'What is this?',
    'header.openSettings': 'Settings',
    'header.model': 'Model',
    'header.modelNotSet': 'No model configured',
    'header.tipProvider': 'Provider',
    'header.tipVision': 'Capability',
    'header.tipVisionYes': 'Vision ✓',
    'header.tipEndpoint': 'Endpoint',
    'header.tipNotSet': 'No model service configured yet. Click to open settings, pick a provider and paste your API key.',
    'header.tipOpenSettings': 'Click to open settings',
    'header.aboutTitle': 'Playground',
    'header.aboutDesc': 'A free-draw canvas for interacting with vision-capable language models. Sketch, reference, annotate, hint — then let the model see and build.',
    'header.aboutProviders': 'Supported Providers',
    'header.aboutHow': 'How it works',
    'header.aboutStep1T': 'Draw or import',
    'header.aboutStep1D': 'Sketch wireframes, paste screenshots, drop reference images.',
    'header.aboutStep2T': 'Describe',
    'header.aboutStep2D': 'Add a text prompt for what you want built.',
    'header.aboutStep3T': 'Generate',
    'header.aboutStep3D': 'The model sees your canvas + prompt and streams HTML/CSS/JS.',
    'header.aboutStep4T': 'Refine',
    'header.aboutStep4D': 'Iterate. The model sees your sketch, the output screenshot, and your feedback.',
    'header.aboutFeatures': 'Features',
    'header.aboutF1': 'Full canvas capture or frame-based region selection',
    'header.aboutF2': 'Real-time streaming with token visualization',
    'header.aboutF3': 'Plan mode: Gaze → Dream → Create',
    'header.aboutF4': 'Save and load drawings',
    'header.aboutFooter1': 'Click the model button in the header to switch providers',
    'header.aboutFooter2': 'Built by PandaGuGu Studio',
    'theme.dark': 'Dark',
    'theme.light': 'Light',
    'theme.toggleDark': 'Switch to dark background',
    'theme.toggleLight': 'Switch to light background',

    'prompt.inspirationArt': 'Generative Art',
    'prompt.useBlueprint': 'Use canvas blueprint',
    'prompt.useBlueprintHint': 'Fill prompt with the current canvas blueprint JSON for AI to generate HTML',
    'prompt.inspirationApp': 'Wireframe → App',
    'prompt.inspirationLanding': 'Landing Page',
    'prompt.inspirationDash': 'Dashboard',
    'prompt.placeholderNew': 'Describe what you want to build…',
    'prompt.placeholderRefine': 'Describe changes, or leave empty and hit Refine…',
    'prompt.planOn': 'Plan mode ON — Gaze, Dream, then Create',
    'prompt.planOff': 'Enable Plan mode for deeper, multi-step generation',
    'prompt.planLabel': 'Plan',
    'prompt.planHint': 'Gaze · Dream · Create',
    'prompt.clear': 'Clear',
    'prompt.noKey': 'No Key',
    'prompt.refine': 'Refine',
    'prompt.generate': 'Generate',
    'prompt.hintNoKey': 'Paste API key above',

    'frame.fullCanvas': 'Full canvas will be sent',
    'frame.drawFirst': 'Draw something to get started',
    'frame.prevOutput': 'previous output',
    'frame.add': 'Frame',
    'frame.save': 'Save',
    'frame.load': 'Load',
    'frame.sources': 'Sources',
    'frame.all': 'All',
    'frame.lastOutput': 'Last Output',
    'frame.preview': 'PRV',
    'frame.collapse': 'Collapse',
    'frame.expand': 'Expand',
    'frame.frames': 'frames',
    'frame.title': 'Frame',
    'frame.adjustHint': 'Resizing the frame does not scale its children',
    'template.title': 'Classic Templates',
    'template.desc': 'Pick a template — a frame with semantic elements is generated on the canvas, ready to edit',
    'template.mobile': 'Mobile',
    'template.desktop': 'Desktop',
    'template.appHome': 'App Home',
    'template.appLogin': 'App Login',
    'template.saasLanding': 'SaaS Landing',
    'template.dashboard': 'Dashboard',
    'template.ecommerce': 'E-commerce',
    'template.portfolio': 'Portfolio',

    'layers.title': 'Layers',
    'layers.add': 'Add frame',
    'layers.delete': 'Delete layer',
    'layers.empty': 'Canvas is empty — draw something to get started',
    'frame.exportJson': 'Export this frame as JSON',

    'app.apiKeyTitle': 'API Key Required',
    'app.apiKeyDesc': 'Get an API key from {url} for {provider}.',
    'app.apiKeyStep1': 'Get a key from {url}',
    'app.apiKeyStep2': 'Click the model button in the header to open settings',
    'app.apiKeyStep3': 'Paste the key in the {provider} card',
    'app.openSettings': 'Open Settings',
    'app.copy': 'COPY',
    'app.openNew': 'OPEN',
    'app.download': 'Download HTML',
    'variant.title': 'Batch Variants',
    'variant.presets': 'Presets',
    'variant.customPlaceholder': 'Custom style, e.g. Japanese magazine layout…',
    'variant.add': 'Add',
    'variant.generate': 'Generate {n} variants',
    'variant.generating': 'Generating…',
    'variant.failed': 'All variants failed — check network or API key',
    'variant.barLabel': 'Variants',
    'variant.download': 'Download this variant',
    'variant.exit': 'Exit variant preview',
    'variant.minimal': 'Minimal',
    'variant.cyberpunk': 'Cyberpunk',
    'variant.morandi': 'Morandi',
    'variant.glass': 'Glassmorphism',
    'variant.terminal': 'Dark Terminal',
    'variant.neumorphic': 'Neumorphic',
    'variant.luxury': 'Luxury',
    'variant.retro': 'Retro Pixel',
    'history.title': 'History',
    'history.empty': 'No history yet — generations are auto-saved here',
    'history.load': 'Load this version into preview',
    'history.download': 'Download this version',
    'history.delete': 'Delete this entry',
    'history.clearAll': 'Clear all history',
    'import.title': 'Import HTML to canvas',
    'import.desc': 'Paste HTML or pick a file. It will be simplified into semantic elements (div→container, h1→heading, button→button…) and auto-laid-out on the canvas for direct editing.',
    'import.file': 'Choose HTML file',
    'import.hint': 'or paste below',
    'import.placeholder': '<!DOCTYPE html>\n<html>\n  <body>\n    <div class="hero">\n      <h1>Hello</h1>\n      <button>Go</button>\n    </div>\n  </body>\n</html>',
    'import.confirm': 'Import to canvas (clears current canvas)',
    'import.cancel': 'Cancel',
    'import.empty': 'Enter HTML first',
    'import.parseError': 'Could not parse HTML — check the content',
    'import.importing': 'Importing…',
    'import.readError': 'File read failed',
    'app.desktop': 'Desktop',
    'app.tablet': 'Tablet',
    'app.mobile': 'Mobile',
    'app.phaseGaze': 'Gaze',
    'app.phaseDream': 'Dream',
    'app.phaseCreate': 'Create',
    'app.chipFullCanvas': 'Full canvas',
    'app.chipCurrentOutput': 'Current output',

    'settings.title': 'Settings',
    'settings.general': 'General',
    'settings.language': 'Language',
    'settings.openaiCompat': 'OpenAI-compat',
    'settings.active': 'ACTIVE',
    'settings.url': 'URL',
    'settings.searchModels': 'Search vision models...',
    'settings.loading': 'loading...',
    'settings.error': 'err',
    'settings.modelIdPlaceholder': 'type model ID...',
    'settings.cancel': 'Cancel',
    'settings.save': 'Save',

    'stream.thinking': 'THINKING',
    'stream.complete': 'COMPLETE',
    'stream.thinkingNow': 'THINKING',
    'stream.streaming': 'STREAMING',
    'stream.chars': 'chr',

    'plan.gazing': 'Gazing into the image…',
    'plan.dreaming': 'Dreaming up the design…',
    'plan.creating': 'Crafting the code…',
    'plan.working': 'Working…',
    'plan.phase': 'Phase',

    'semantic.title': 'Semantic',
    'semantic.autoTag': 'Auto-tag',
    'semantic.drawHint': 'No selection — click then drag on canvas to create',
    'align.title': 'Align',
    'align.needMulti': 'Select multiple elements first (drag-select or Shift+click)',
    'align.left': 'Align left',
    'align.hcenter': 'Center horizontally',
    'align.right': 'Align right',
    'align.top': 'Align top',
    'align.vcenter': 'Center vertically',
    'align.bottom': 'Align bottom',
    'align.hdistribute': 'Distribute horizontally',
    'align.vdistribute': 'Distribute vertically',
    'semantic.container': 'Container',
    'semantic.section': 'Section',
    'semantic.card': 'Card',
    'semantic.nav': 'Nav',
    'semantic.heading': 'Heading',
    'semantic.text': 'Text',
    'semantic.link': 'Link',
    'semantic.button': 'Button',
    'semantic.input': 'Input',
    'semantic.image': 'Image',
    'semantic.raw': 'Raw HTML',
    'semantic.note': 'Note',
    'semantic.groupContainer': 'layout',
    'semantic.groupContent': 'content',
    'semantic.groupControl': 'control',
    'semantic.groupMedia': 'media',
    'semantic.groupSpecial': 'special',
    'semantic.untag': 'Clear tag',
    'semantic.search': 'Search and tag',
    'semantic.searchPlaceholder': 'Search types…',
    'semantic.searchEmpty': 'No matching type',
    'semantic.exportBlueprint': 'Export JSON (embedded images)',
    'semantic.exportBlueprintFile': 'Export JSON (image filenames)',
    'semantic.exportBlueprintFileHint': 'Images referenced by filename (e.g. image-1.png), no base64 — pass image files to the AI separately',
    'semantic.exportHtml': 'Export HTML (images inlined)',
    'semantic.exportHtmlHint': 'Generate a self-contained HTML with base64 images — open it directly, no AI needed',
    'semantic.copyForAI': 'Copy prompt for web AI',
    'semantic.copyForAIHint': 'Copies prompt+JSON (tells the AI the src is a base64 string — copy verbatim, do not decode)',
    'semantic.exportPng': 'Export canvas PNG',
    'semantic.exportMenu': 'Export options',
    'semantic.exportEmpty': 'No semantic tags on canvas yet. Select a shape and tag it first.',
    'semantic.importHtml': 'Import HTML',
    'semantic.templates': 'Templates',
    'semantic.propsHint': 'props',
    'semantic.layout': 'Layout',
    'semantic.layoutFree': 'Free',
    'semantic.layoutRow': 'Row',
    'semantic.layoutColumn': 'Column',
    'semantic.layoutGrid': 'Grid',
    'semantic.layoutWrap': 'Wrap',
    'semantic.advanced': 'advanced',
    'semantic.style': 'Free CSS (style)',
    'semantic.stylePlaceholder': 'display:flex; gap:16px; backdrop-filter:blur(10px)',
    'semantic.events': 'Events (JSON)',
    'semantic.htmlHint': 'This type edits html directly in the main form.',
    'props.label': 'Label',
    'props.width': 'Width',
    'props.height': 'Height',
    'props.bg': 'Background',
    'props.radius': 'Radius',
    'props.padding': 'Padding',
    'props.border': 'Border',
    'props.shadow': 'Shadow',
    'props.content': 'Content',
    'props.fontSize': 'Font size',
    'props.fontWeight': 'Weight',
    'props.color': 'Color',
    'props.bold': 'Bold',
    'props.align': 'Align',
    'props.alignLeft': 'Left',
    'props.alignCenter': 'Center',
    'props.alignRight': 'Right',
    'props.alt': 'Alt',
    'props.fileName': 'File name',
    'props.src': 'Image URL',
    'props.href': 'Href',
    'props.placeholder': 'Placeholder',
    'props.type': 'Type',
    'props.level': 'Level',
    'props.fit': 'Fit',
    'props.html': 'HTML',
  },
}

interface TParams {
  [key: string]: string
}

type TFunction = (key: string, params?: TParams) => string

const I18nContext = createContext<TFunction>(() => '')

export function I18nProvider({ lang, children }: { lang: Lang; children: React.ReactNode }) {
  const t: TFunction = (key, params) => {
    const table = translations[lang]
    let s = (table && table[key]) || key
    if (params) {
      for (const [k, v] of Object.entries(params)) {
        s = s.replace(new RegExp(`\\{${k}\\}`, 'g'), v)
      }
    }
    return s
  }
  return <I18nContext.Provider value={t}>{children}</I18nContext.Provider>
}

export function useI18n(): TFunction {
  return useContext(I18nContext)
}
